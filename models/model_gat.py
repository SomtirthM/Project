...
class PSENet(nn.Module):
    def __init__(self, backbone, result_num=6, scale: int = 1, pretrained=False):
        ...
        self.backbone = model(pretrained=pretrained, out=out)
        # Reduce channels
        # Top layer
    ...
    def forward(self, input: torch.Tensor):
        ...
        import numpy as np
        p5 = self.toplayer(c5)
        np.save("npy_files_gat/p5.npy", p5.cpu().numpy())
        p4 = self._upsample_add(p5, self.latlayer1(c4))
        p4 = self.smooth1(p4)
        np.save("npy_files_gat/p4.npy", p4.cpu().numpy())
        p3 = self._upsample_add(p4, self.latlayer2(c3))
        p3 = self.smooth2(p3)
        np.save("npy_files_gat/p3.npy", p3.cpu().numpy())
        p2 = self._upsample_add(p3, self.latlayer3(c2))
        p2 = self.smooth3(p2)
        np.save("npy_files_gat/p2.npy", p2.cpu().numpy())

        x = self._upsample_cat(p2, p3, p4, p5)
        x = self.conv(x)
        x = self.out_conv(x)
        np.save("npy_files_gat/x_output.npy", x.cpu().numpy())
